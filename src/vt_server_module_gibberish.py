...
created out of sentence chunks, that can be used in the CRM experiment.
...
def process_gibberish(in_filename, m, out_filename):
	...
	while d < m['total_dur']:
		...
		if d==0:
			masker = np.zeros((int(m['total_dur']*fs),y.shape[1]))
			fs_ref = fs
			masker_i = 0
		else:
			if fs!=fs_ref:
				y = scipy.signal.resample(y, int(y.shape[0]*fs_ref/fs))
				fs = fs_ref
...
if __name__ == '__main__':
	...
	m['module'] = 'gibberish'
	m['seed'] = 1
	m['shell_pattern'] = {'include': 'cat*.wav'}
	m['chunk_dur_min'] = .4
	m['chunk_dur_max'] = .5
	m['total_dur'] = 2
	m['ramp'] = .05
